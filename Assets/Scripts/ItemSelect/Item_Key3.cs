...
public class Item_Key3 : MonoBehaviour {
	...
	public void Click () {
		...
		GameObject gameObject = this.transform.Find ("key3_x").gameObject; 
		...
		if (FlagManager.Instance.flags [7] == false && FlagManager.Instance.flags [8] == false) {
			...
			FlagManager.Instance.flags [44] = true;
			...
			GameObject gameObjectA = this.transform.Find ("TextKey3").gameObject;
